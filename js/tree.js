/* global timeGraphPromise d3 whenResize */
timeGraphPromise.then(
  ({ PhylumClassOrderFamilyGenusSpecies, kdeDatum, reHighlight, redrawBarByX, marginTime }) => {
    ...
    const setWidthHeight = () => {
      let { width: widthTemp, height: heightTemp } = tree.getBoundingClientRect()
      heightTemp -= marginTree.top + marginTree.bottom
      widthTemp -= marginTree.left
      ;[width, height] = [widthTemp, heightTemp]
      d3.partition().size([height, ((root.height + 1) * width) / 2])(root)
      svg
        .attr('viewBox', [
          0,
          0,
    ...
    whenResize(() => {
      ...
      svg.selectAll('.tree-rect').attr('height', (d) => rectHeight(d.target))
      svg
        .selectAll('.tree-text')
        .attr('fill-opacity', (d) => +labelVisible(d.target))
        .attr('y', (d) => rectHeight(d.target) / 2 + 5)
