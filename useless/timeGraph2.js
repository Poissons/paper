d3.csv("./data/data_early.csv").then(function (early_csv) {
    ...
    let early_data=early_csv.map(d=>{
        return{
            ...d,
    ...
    // let early_csv_species_name=[...new Set(early_csv.map(obj => obj['species_name']))];

    let early_csv_species_name = d3.nest().key(d=>d.species_name).entries(early_data).map(d=>d.key);

    let color = d3.scaleOrdinal(d3.schemeSet2).domain(early_csv_species_name);
  ...
  el
    ...
    let dataBySpeciesName = d3.nest().key(d=>d.species_name).entries(early_data);
    ...
    let filteredData;
    filteredData = [].concat.apply([], dataBySpeciesName.map(d=>d.values));
